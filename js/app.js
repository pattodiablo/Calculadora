var calculadora = {
	...
	asignarEventosFormatoBotones: function(selector){
		...
		for (var i = 0; i<x.length;i++) {
			x[i].onmouseover = this.HoverButton;
			x[i].onmouseleave = this.mouseOutButton;
		};
	},

	HoverButton: function(event){
		calculadora.smallBtn(event.target);
	},
	...
	ingresarOperaciones: function(){
		...
		document.getElementById("on").addEventListener("click", function() {calculadora.eraseDisplay();});
